...
<p align="center">
  <b>One package. One install. CLI for the agent, Studio for the human. 51 skills, 5 research agents, brand memory that compounds.</b>
</p>
...
|---|---|---|
| `mktg` CLI | 51 skills, 5 agents, brand memory, `/cmo` orchestration | (root) |
...
`marketing-cli` fixes that. Install once and your agent inherits 51 skills, 5 research agents, and a brand memory that compounds. Session 10 starts where session 9 ended. Every conversation builds on the last.
...
| **51 marketing skills** | The playbook, copied to `~/.claude/skills/` |
...
That's it. The global npm install copies the 51 skills into `~/.claude/skills/` and the 5 agents into `~/.claude/agents/`. No second install command, no `mktg init` to remember.
...
| Skill | What it does |
|-------|-------------|
| **cmo** | Orchestrates all 51 skills. Routing table, disambiguation, guardrails. |
...
| `mktg list` | Show all 51 skills with install status and metadata |
...
- [Corey Haines' Marketing Skills](https://github.com/coreyhaines31/marketingskills): the breadth skill collection that inspired many of mktg's 51 skills
