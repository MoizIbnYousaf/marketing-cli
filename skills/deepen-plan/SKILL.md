...
name: deepen-plan
description: |
  Enhance an existing marketing plan with parallel research. Use when the agent has a draft plan (brand strategy, campaign brief, content calendar, launch plan) that needs strengthening with real data. Triggers on 'deepen this plan', 'strengthen this strategy', 'research gaps in my plan', 'make this plan better', 'this plan is too surface level', 'add research to this', or when a plan exists but lacks audience data, competitive positioning, or keyword strategy.
allowed-tools: []
---
...
- Tell it: "Research the target audience for [product]. The current plan says [quote relevant section]. Find buyer personas, where they hang out online, what language they use, and what objections they have."
...
- Tell it: "Research competitors for [product] in the [space] market. The current plan mentions [competitors if any]. Find 3-5 competitors, their positioning, pricing, strengths, and weaknesses."
...
|-------------|---------|
| Rewriting the user's plan from scratch | Add to existing sections, preserve original thinking |
| Running all 5 research types regardless | Only research actual gaps — strong sections don't need more data |
| Spawning agents for keyword or distribution gaps | Keywords need `/keyword-research` skill. Distribution is synthesized from audience data |
| Deepening a plan that doesn't exist yet | Route to `/brainstorm` or `/launch-strategy` first |
| Running without showing the gap analysis | Always show the gap table and explain what you'll research |
| Silently updating brand files | Agents write to brand/. That's fine. But tell the user what was updated |
| Deepening the same plan twice without new context | Flag it: "This plan was already deepened on [date]. Want me to research something specific, or has the situation changed?" |
