...
description: >
  ...
  Detects gws configuration for direct email sending. Loads brand voice and positioning from
...
1. **Check `./brand/stack.md`** (if exists) -- look for email tool entries in the Connected Tools table.
2. **Check for `gws` CLI** -- verify gws is available for direct email sending.
...
**If gws detected:**
```
Email Tool Detection
├── gws               ✓ available
│
│   I can send emails directly via gws.
│   Want me to set it up, or output
│   copy-paste-ready files?
│
│   → "Set it up"     Send via gws
