# E-E-A-T Content Examples

Best-in-class examples of human-written content that demonstrates Experience,
Expertise, Authoritativeness, and Trust. Study these patterns.

---

## What Makes Content "Human"

1. **A specific person with a specific viewpoint** -- not a committee
2. **Real experience showing through** -- stories, mistakes, specific numbers
3. **Opinions defended with reasoning** -- not hedged, not "some say X, others say Y"
4. **Unique insight or data** -- something you can't find by aggregating other sources
5. **Voice that could only be this person** -- recognizable even without a byline

---

## Example 1: Paul Graham

**Source:** paulgraham.com essays

**Why it works:** Unmistakable point of view backed by decades of startup experience.

**What to steal:**
- Direct statements ("all you need to do is")
- Acknowledging the real world ("in practice")
- Contrarian reframing ("already too conservative")
- Building an argument step by step

---

## Example 2: Wait But Why (Tim Urban)

**Source:** waitbutwhy.com

**Why it works:** Deep research made accessible through personality.

**What to steal:**
- Specific sourcing ("I have a friend who's a physicist")
- Extending analogies to reveal insight
- Admitting "we have no idea" when true
- Making complex ideas visceral

---

## Example 3: Stratechery (Ben Thompson)

**Source:** stratechery.com

**Why it works:** Daily analysis with a consistent analytical framework (Aggregation Theory).

**What to steal:**
- Original frameworks with clear definitions
- Building from principles to applications
- Historical context that illuminates current events
- Confidence in analysis (not hedged to death)

---

## Example 4: James Clear

**Source:** jamesclear.com articles

**Why it works:** Research backing AND personal application.

**What to steal:**
- Specific research citations with details (248 people, 66 days)
- Debunking common misconceptions with evidence
- Storytelling structure (setup, surprise, reframe)
- Making research findings immediately relevant

---

## Example 5: Backlinko (Brian Dean)

**Source:** backlinko.com

**Why it works:** Every tactic backed by data from his own experiments.

**What to steal:**
- Specific results with numbers (110%, 14 days)
- Personal narrative arc (struggled, discovered, succeeded)
- "I'll show you exactly how" -- and then actually does
- Setting up the transformation story

---

## Example 6: Lenny's Newsletter (Lenny Rachitsky)

**Source:** lennysnewsletter.com

**Why it works:** Primary research from interviewing practitioners.

**What to steal:**
- Primary research methodology stated explicitly (50 leaders, specific companies)
- Honest results even when they contradict expectations
- Named sources with real quotes
- Going deeper when initial answer is unsatisfying

---

## Example 7: Derek Sivers

**Source:** sive.rs

**Why it works:** Short, punchy essays with unconventional positions defended by experience.

**What to steal:**
- Strong position stated immediately
- Simple framework that's memorable
- No hedging
- Personal application implied throughout

---

## Example 8: Matt Levine (Finance)

**Source:** Bloomberg Money Stuff newsletter

**Why it works:** Complex financial concepts with humor and clarity.

**What to steal:**
- Complex topics explained through everyday analogies
- Willingness to state conclusions clearly
- Dry humor that doesn't undermine substance
- Building arguments by showing what's missing

---

## Example 9: J. Kenji Lopez-Alt (Food Science)

**Source:** Serious Eats, The Food Lab

**Why it works:** Scientific methodology applied to cooking. Tests dozens of variations.

**What to steal:**
- Specific numbers from actual tests (50 steaks, 30 seconds, 30% faster)
- Challenging conventional wisdom with evidence
- Methodology transparency
- Practical conclusions from scientific approach

---

## Example 10: Julia Evans (Technical/Programming)

**Source:** jvns.ca, Wizard Zines

**Why it works:** Complex systems topics made approachable without dumbing down.

**What to steal:**
- Starting from genuine curiosity
- Admitting uncertainty
- Specific scope ("spent a week")
- Inviting engagement

---

## Example 11: Dr. Peter Attia (Medicine)

**Source:** The Drive podcast, peterattiamd.com

**Why it works:** Practitioner depth. Actually reads the papers, explains the statistics.

**What to steal:**
- Actually reading the primary sources
- Distinguishing relative vs absolute risk
- Teaching methodology, not just conclusions
- Nuanced positions

---

## Example 12: Morgan Housel (Behavioral Finance)

**Source:** "The Psychology of Money"

**Why it works:** Money and decision-making through psychology and history.

**What to steal:**
- Specific contrasting stories
- Simple conclusions from complex data
- Redefining success
- No jargon despite financial topic

---

## Example 13: Dan Luu (Engineering)

**Source:** danluu.com

**Why it works:** Deep technical analyses backed by actual measurement.

**What to steal:**
- Measurement over assumption
- Historical comparison that reveals regression
- Specific numbers that can be verified
- Clear conclusion that challenges consensus

---

## The Pattern Across All Examples

1. **First-Person Experience** -- "I found" not "studies show"
2. **Specific Numbers** -- "248 people" not "many"
3. **Named Sources** -- real people, real companies, real quotes
4. **Contrarian Angles** -- challenging convention with evidence
5. **Admitted Limitations** -- "this won't work for everyone"
6. **Clear Voice** -- identifiable without a byline
7. **Original Insight** -- can't be found by aggregating other sources

---

## How to Apply

When writing SEO content, ask:

1. What's my specific experience with this topic?
2. What numbers can I cite from actual work?
3. What's my actual opinion here?
4. What would I tell a smart friend who asked about this?
5. What can only I say about this?

If you can't answer these, you're writing AI-style content even if AI didn't write it.

---

## Quick Reference by Vertical

| Vertical | Examples | Key Patterns |
|----------|----------|--------------|
| Marketing/Business | Paul Graham, Derek Sivers, Backlinko, Lenny | Conviction, methodology, primary research |
| Finance | Matt Levine, Morgan Housel | Complex to accessible, stories over jargon |
| Tech/Engineering | Julia Evans, Dan Luu | Measurement, curiosity, admitting failures |
| Healthcare | Dr. Peter Attia | Primary sources, nuance, human stakes |
| Food/Consumer | J. Kenji Lopez-Alt | Scientific method, myth-busting with data |
| Deep Explainers | Wait But Why, Stratechery | Research depth, original frameworks |
