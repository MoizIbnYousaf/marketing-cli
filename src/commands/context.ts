...
const parseContextFlags = (args: readonly string[]): { layer?: string; budget?: number; save: boolean } => {
  ...
  return { layer, budget, save };
};

export const handler: CommandHandler<ContextResult> = async (args, flags) => {
  const cwd = flags.cwd;
  const { layer, budget, save } = parseContextFlags(args);
