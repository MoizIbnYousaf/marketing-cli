...
export const schema: CommandSchema = {
  ...
  output: {
    ...
    tasks: "PlanTask[] — prioritized task queue",
...
const buildTasks = async (
  ...
  if (!hasBrand) {
    tasks.push({
      id: "init-brand", priority: priority++, category: "setup",
  ...
  for (const file of [...foundationOrder, ...strategyOrder, ...configOrder]) {
    ...
    if (!status?.exists) {
      tasks.push({
        id: `create-${file.replace(".md", "")}`, priority: priority++, category: "setup",
    ...
    try {
      ...
      if (isTemplateContent(file, content)) {
        ...
        tasks.push({
          id: `populate-${file.replace(".md", "")}`, priority: priority++, category: "populate",
  ...
  for (const status of brandStatuses) {
    if (status.freshness === "stale") {
      tasks.push({
        id: `refresh-${status.file.replace(".md", "")}`, priority: priority++, category: "refresh",
  ...
  try {
    ...
    for (const skill of mustHaveSkills) {
      if (!runSummary[skill]) {
        tasks.push({
          id: `run-${skill}`, priority: priority++, category: "execute",
  ...
  if (hasContent && !hasDistributed) {
    tasks.push({
      id: "distribute-content", priority: priority++, category: "distribute",
...
export const handler: CommandHandler<PlanResult | { completed: string } | { task: PlanTask | null }> = async (args, flags) => {
  ...
  const { tasks, health } = await buildTasks(cwd, runSummary, completed);
  ...
  if (isTTY() && !flags.json) {
    ...
    for (const task of tasks.slice(0, 10)) {
      ...
      writeStderr(`  ${icon} ${bold(`#${task.priority}`)} ${task.action}${blockedTag}`);
