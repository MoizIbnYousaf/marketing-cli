...
// ─── Template detection ─────────────────────────────────────────────────────
//
// Align with the CLI's `isTemplateContent()`: exact seed match wins.
// A char-count heuristic alone was too aggressive — short but real voice
// profiles (a few hundred chars) were falsely labeled "template".
//
// Signals (either is enough):
//   1. Exact hash match against the mktg-CLI seed at TEMPLATE_DIR
//   2. Presence of the `<!-- Generated by mktg init` scaffold marker
//      (fallback when seed templates aren't on disk)

/** Marker embedded in every `mktg init` scaffold. */
const INIT_MARKER = "<!-- Generated by mktg init";
 ...
 *
 * Never returns the *project's* own `brand/` directory. When the checkout
 * itself is `marketing-cli` (or a symlink to it), that path is the live
 * project memory — comparing a file to itself would mark every file as a
 * template.
 */
 ...
 * Exact seed-hash match (CLI parity), or the mktg init scaffold marker.
 */
export function looksLikeTemplate(file: string, content: string): boolean {
  ...
  if (seedPath && existsSync(seedPath)) {
    try {
      ...
      TEMPLATE_HASHES.add(seedHash);
      if (hash === seedHash) return true;
    } catch {
      // missing/permissions issues fall through to the marker heuristic
    }
  }
  return content.includes(INIT_MARKER);
}
