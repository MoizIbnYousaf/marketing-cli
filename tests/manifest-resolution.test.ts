...
describe("resolveManifest", () => {
  ...
  test("real package manifest has 39 skills", async () => {
    const manifest = await loadManifest();
    expect(Object.keys(manifest.skills)).toHaveLength(39);
  });

  test("project with 1 new skill results in 40 total skills", async () => {
    const projectManifest = {
      version: 1,
      skills: {
        "extra-skill": {
    ...
    const result = await resolveManifest(tmpDir);
    expect(Object.keys(result.skills)).toHaveLength(40);
  });
});
