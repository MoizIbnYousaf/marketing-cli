...
describe("Full pipeline: init → status → doctor → list → update", () => {
  ...
  test("Step 6: list shows all 35 skills", async () => {
    const result = await listHandler([], flags);
    expect(result.ok).toBe(true);
    if (!result.ok) return;

    expect(result.data.total).toBe(35);
    expect(result.data.skills).toHaveLength(35);
  ...
  test("Step 7: update reports no changes on fresh install", async () => {
    ...
    expect(result.data.totalSkills).toBe(35);
    expect(result.data.skills.unchanged.length + result.data.skills.updated.length + result.data.skills.notBundled.length).toBe(35);
  });
